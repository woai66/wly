...
bool Confirm_OK()
{
    ...
    {
        Reset_Navigation();
        Left_Board_FSM.CurState = Move_Left;//如果移动没到位，就再进行一次调整
        return false;
    }
    return true;
}
...
{
    //当前的状态		当前状态执行函数		跳转事件条件函数		下一个状态
	{Find_Left,         Change_Direction,     TurnAction_Finish,      Wait_Data       },
    {Wait_Data,         Car_Stop_Wait_Data,   Data_Receive,           Move_Left       },
    {Move_Left,         Move_Action,          Leftmove_Finish,        Confirm_Left    },
    {Confirm_Left,      Car_Stop_Wait_Data,   Confirm_OK,             Return_Line     },
...
{
    //当前的状态		当前状态执行函数		跳转事件条件函数		下一个状态
	{Find_Left,         Change_Right,         TurnAction_Finish,      Wait_Data       },
    {Wait_Data,         Car_Stop_Wait_Data,   Data_Receive,           Move_Left       },
    {Move_Left,         Move_Action,          Leftmove_Finish,        Confirm_Left    },
    {Confirm_Left,      Car_Stop_Wait_Data,   Confirm_OK,             Return_Line     },
...
void My_FSM_Init()
{
    ...
    FSMInit(&Left_Board_FSM,Find_Left,LeftBoardTable,1);
    FSMInit(&Right_Board_FSM,Find_Left,RightBoardTable,2);
}
