...
#define FLASH_PAGE_INDEX (FLASH_PAGE_SIZE)// 存储数据用的页码 倒数第一个页码
...
static void Page0_Mode()
{
    ...
    if(!Menu.Image_Show)
    {
        ...
        tft180_show_string(Row_1,Line_4,"mx:");
        tft180_show_float(Row_4,Line_4,Get_X_Distance(),3,1);
...
void Menu_Display()
{
    ...
    tft180_show_gray_image(0, 0, (const uint8 *)(Bin_Image), MT9V03X_W, MT9V03X_H, (MT9V03X_W), (MT9V03X_H), 0);
    tft180_show_int(Row_8, Line_6,timer_get(TIM_1),3);
//    switch(Menu_Mode)
//    {
//        case Page_Select:
//            Page_Select_Mode();
