...
class BMP280():
    ...
    def get(self):
        adc_T = (self.getReg(0xFA) << 12) + \
            (self.getReg(0xFB) << 4) + (self.getReg(0xFC) >> 4)
        ...
        adc_P = (self.getReg(0xF7) << 12) + \
            (self.getReg(0xF8) << 4) + (self.getReg(0xF9) >> 4)
    ...
    def getAltitude(self):
        #hypsometric 公式，单位kpa
        return ((101325/self.getPress())**(1/5.257)-1)*(self.getTemp()+273.15)*153.846

    def getAltitudeb(self):
        #barometric 公式，不考虑温度影响
